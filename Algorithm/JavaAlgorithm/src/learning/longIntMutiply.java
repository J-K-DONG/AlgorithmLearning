...
class bigIntMult {
    ...
    // 大整数减法：num1 被减数，大整数乘法中只有一个减法(A+B)(C+D)-(AC+BD)=AD+BC>0,因此參數num1>num2且都为正  num2 减数
    ...
    public char[] mult(char[] num1, char[] num2) {
        char[] A, B, C, D, AC, BD, A_B, C_D, AC_BD, A_B_C_D,  SUM;
        ...
        if (num1.length > 1) {
            ...
            A_B = add(A, B);  // A+B
            C_D = add(C, D);  // C+D
            AC_BD = add(AC, BD); // AC + BD
            A_B_C_D = mult(A_B, C_D);  // (A+B) * (C+D)
            // (A+B)(C+D)-(AC+BD)=AD+BC
            char[] tmp1 = addTail(sub(A_B_C_D, AC_BD), N / 2);//尾部补0，相当于移位
    ...
    public static void main(String[] args) {
        ...
        String st2 = sc2.next();
//        String st1 = "99999999999999999999";
//        String st2 = "88888888888888888888";

        char[] a = st1.toCharArray();
        char[] b = st2.toCharArray();
        bigIntMult bg = new bigIntMult();
        char[] res = bg.mult(a, b);
